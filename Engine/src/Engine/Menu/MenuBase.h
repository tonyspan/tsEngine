...
namespace tsEngine
{
	...
	protected:
		...
		Ref<T> MenuContext()
		{
			if constexpr (std::is_same<T, AudioManager>::value)
				return Application::Get().m_AudioManager;
			else if constexpr (std::is_same<T, EntityManager>::value)
				return Application::Get().m_EntityManager;
			else if constexpr (std::is_same<T, RenderManager>::value)
				return Application::Get().m_RenderManager;
			else
				static_assert(false, "MenuContext: Wrong context");
		}
