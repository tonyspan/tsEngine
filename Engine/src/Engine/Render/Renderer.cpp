...
namespace tsEngine
{
		...
		: m_Window(window)
	{
		...
		m_ImGuiTexture = Texture::Create(this, m_Window->GetWidth(), m_Window->GetHeight());
	...
	void Renderer::RenderEntities(const Ref<EntityManager>& entityManager, const CameraData& camera)
	{
		...
		{
			SDL_SetRenderTarget(m_NativeRenderer.get(), m_ImGuiTexture->Raw());
			...
			if (IsInsideScreen(transform.Position, size, camera))
			{
				...
				if (sprite.Image)
				{
					...
					if (sprite.Animation)
					{
						...
						SDL_RenderCopyEx(m_NativeRenderer.get(), sprite.Image->Raw(), &src, &dst, transform.Rotation, NULL, flip);
					}
					else
					{
						SDL_RenderCopyEx(m_NativeRenderer.get(), sprite.Image->Raw(), NULL, &dst, transform.Rotation, NULL, flip);
					}

					SDL_SetTextureColorMod(sprite.Image->Raw(), sprite.Color.r, sprite.Color.g, sprite.Color.b);
				}
				else
				{
		...
		for (auto entity : view4)
		{
			...
			if (IsInsideScreen(transform.Position, transform.Size, camera))
			{
				...
				SDL_Surface* surface = TTF_RenderText_Solid_Wrapped(text.Font->Raw(), text.Text.c_str(), color, text.WrapWidth);
				...
				SDL_Rect dst = { (int)screenPosition.x - size.x / 2, (int)screenPosition.y - size.y / 2, size.x, size.y };
		...
		// Rendering to texture is done, copy the texture to the screen
		{
			...
			SDL_RenderCopy(m_NativeRenderer.get(), m_ImGuiTexture->Raw(), NULL, NULL);
		}
	}

	Ref<Texture> Renderer::GetImGuiTexture() const
	{
	...
	void Renderer::OnResize(uint32_t width, uint32_t height)
	{
		...
		m_ImGuiTexture = Texture::Create(this, width, height);
	}
