...
namespace tsEngine
{
	...
	void Serializer::SerializeEntity(YAML::Emitter& out, entt::entity entity)
	{
		...
		if (m_Context->HasComponent<SpriteComponent>(entity))
		{
			...
			if (sc.Image)
				out << YAML::Key << "Image" << YAML::Value << std::filesystem::relative(sc.Image->Path, AssetManager::s_BasePath).string();
			else
			...
			out << YAML::Key << "Animation" << YAML::Value << sc.Animation;
		...
		if (m_Context->HasComponent<TextComponent>(entity))
		{
			...
			out << YAML::Key << "Font" << YAML::Value << std::filesystem::relative(tc.Font->Path, AssetManager::s_BasePath).string();
			...
			for (auto entity : entities)
			{
				...
				if (spriteComponent)
				{
					...
					if (!texNode.IsNull())
					{
						...
						auto tex = Texture::Create(Renderer::s_Instance, fullPath.string());
						AssetManager::AddTexture(texPath, tex);

						src.Image = AssetManager::GetTexture(texPath);
					}
					...
					src.Animation = spriteComponent["Animation"].as<bool>();
				...
				if (textComponent)
				{
					...
					auto fontSize = Font::Create(fullPath.string(), textComponent["FontSize"].as<int>());
					AssetManager::AddFont(fullPath.string(), fontSize);
					
					tc.Font = AssetManager::GetFont(fullPath.string());
